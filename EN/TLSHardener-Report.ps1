...
.SYNOPSIS
    ...
    Visual HTML report of system security configuration.

.DESCRIPTION
    This script generates a detailed and visual report of TLS/SSL security configuration.
    
    Report content:
    - Protocol settings (TLS 1.0-1.3, SSL 2.0/3.0)
    - Encryption algorithms (Ciphers)
    - Hash algorithms (MD5, SHA family)
    - Key exchange algorithms (DH, ECDH, PKCS)
    - Cipher Suite ordering
    - ECC Curves configuration
    - FIPS policy
...
.PARAMETER OpenReport
    Automatically opens the report in default browser after generation

.EXAMPLE
    .\TLSHardener-Report.ps1
    Generates report and saves to reports/ folder
...
.INPUTS
    None. This script does not accept input parameters.

.OUTPUTS
    HTML file: reports/TLSHardener_Report_YYYY_MM_DD_HHMM.html
...
# Retrieving data from Registry and generating report
# Function: Get Functions and EccCurves Values
...
# Protocol statistics (Only count main protocols - without Client/Server separation)
$protocolNames = @{}
foreach ($entry in $reportData["Protocols"]) {
    # Extract protocol name (TLS 1.2, SSL 3.0, etc.)
    if ($entry.Path -match "Protocols\\([^\\]+)\\") {
        ...
        # Enabled: 1 or 4294967295 = Active
        ...
        # Enabled: 0 or DisabledByDefault: 1 = Disabled
...
foreach ($entry in $reportData["Ciphers"]) {
    $stats.TotalCiphers++
    # Enabled: 4294967295 (0xFFFFFFFF) or 1 = Active
...
<head>
    ...
    <title>TLSHardener - Security Configuration Report - $env:COMPUTERNAME</title>
    <style>
        :root {
            --primary: #667eea;
            --primary-dark: #5a67d8;
    ...
    <div class="container">
        <div class="header">
            <h1>
                ...
                TLSHardener Security Report
            </h1>
            <p class="header-subtitle">SChannel Security Configuration and Protocol Analysis</p>
            <div class="server-info">
                <div class="server-info-item">
                    ...
                    <span><strong>Server:</strong> $env:COMPUTERNAME</span>
                </div>
                <div class="server-info-item">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    ...
                    <span><strong>Report Date:</strong> $(Get-Date -Format "dd MMMM yyyy HH:mm")</span>
                </div>
                <div class="server-info-item">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <div class="stats-grid">
            <div class="stat-card success">
                ...
                <div class="stat-label">Enabled Protocols (/$($stats.TotalProtocols))</div>
            </div>
            <div class="stat-card danger">
                <div class="stat-icon">🚫</div>
                <div class="stat-value">$($stats.DisabledProtocols)</div>
                <div class="stat-label">Disabled Protocols</div>
            </div>
            <div class="stat-card info">
                <div class="stat-icon">🔐</div>
                <div class="stat-value">$($stats.EnabledCiphers)</div>
                <div class="stat-label">Enabled Ciphers (/$($stats.TotalCiphers))</div>
            </div>
            <div class="stat-card warning">
                <div class="stat-icon">🛡️</div>
                <div class="stat-value">$($stats.EnabledHashes)</div>
                <div class="stat-label">Enabled Hashes (/$($stats.TotalHashes))</div>
            </div>
        </div>
        ...
        <div class="search-container">
            ...
            <input type="text" id="searchBox" placeholder="Search registry path or value...">
        </div>
"@
...
$categoryConfig = @{
    ...
    "StrongCrypto" = @{
        Title = ".NET Framework Encryption"
...
function Get-StatusBadge {
    ...
    elseif ($Status -eq "NotFound") {
        return '<span class="status-badge status-notfound"><span class="status-dot"></span>Default</span>'
    }
    else {
    ...
    $reportHtml += @"
        <div class="category-section">
            <div class="category-header $($config.Class)" onclick="toggleSection(this)">
                ...
                <span class="category-badge">$itemCount items</span>
...
# Cipher Suites and ECC Curves section
$reportHtml += @"
        <div class="category-section">
            <div class="category-header ciphersuites" onclick="toggleSection(this)">
                ...
                <span class="category-title">Cipher Suites & ECC Curves <small style="color: var(--gray-500);">(Encryption Packages)</small></span>
                <svg class="category-chevron" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <polyline points="6 9 12 15 18 9"/>
                </svg>
            </div>
...
foreach ($entry in $reportData2["Functions"]) {
    ...
    if ($index -eq 1) {
        $reportHtml += '<div class="cipher-item">Not Configured (System Defaults)</div>'
    }
    ...
    if (-not ($eccCurvesList | Where-Object { $_ -and $_ -ne "NotConfigured" -and $_ -ne "NotFound" })) {
        $reportHtml += '<div class="ecc-item" style="background: var(--gray-100); color: var(--gray-600);">Default</div>'
    }
...
$reportHtml += @"
        ...
        <div class="info-box">
            ...
            <div>
                <strong>Note:</strong> Settings marked as "Default" are not defined in the registry and use operating system defaults.
                For security, it is recommended to explicitly configure all protocols and algorithms.
            </div>
        </div>
        
        <!-- Footer -->
        <div class="footer">
            <p>🛡️ Generated with <strong>TLSHardener</strong> v3.5</p>
            <p style="margin-top: 0.5rem;">Report Date: $(Get-Date -Format "dd MMMM yyyy HH:mm:ss")</p>
        </div>
    </div>
</body>
</html>
